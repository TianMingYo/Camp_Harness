...
- **Stable-reference automation:** commit `9ad13b43f1c46d93e2535050eaa8c7e74982c4ba` passed pull-request run `31565365499` and push run `31565365171`. Both quality jobs passed Docker build and dynamic-port smoke; PR publish was skipped; push artifact save/upload, load, login, and publish all passed. Anonymous `latest` returned HTTP `200` with digest `sha256:14c030d2e166e0f598e8041639d3e1e855b12ae5a9e95d6fb857e9b89f74e455`.
- **Final image transfer deviation:** two direct VPS pulls of that digest reused completed layers but failed on the remaining GitHub CDN layer, first at the approved 600-second cap and then with `read tcp ... 185.199.110.154:443: connection timed out`. The old verified container stayed online. Rather than retry blindly, the controller downloaded run `31565365171` artifact `feedbackloop-demo-image` (`id=9129256600`, not expired), which is the exact image saved after the successful CI smoke. The artifact ZIP SHA-256 was `7f0ae11468d0206782e6ced3045fa3f4982523af1113685752015138fce74153`; its Docker TAR SHA-256 was `c565a656dd8b30281c1306b94c4fab7c7f77d71848c9600aeb55c6d03d206965`. Both hashes matched before and after SCP upload. `docker load` produced image ID `sha256:4fae6c46a6b8a945828da6a9800a21c0c7516bd96ad240bdf0e975c04e0611e8`.
- **Approved final replacement:** after separate student approval, the controller replaced only `feedbackloop-harness`. The command had an automatic rollback to the previous `c51472d...` digest if the new container did not return local HTTP `200`. The first two startup-window requests were empty/reset; the retry succeeded, so no rollback occurred. Final inspection reported `running`, image ID `sha256:4fae6c46a6b8a945828da6a9800a21c0c7516bd96ad240bdf0e975c04e0611e8`, restart policy `unless-stopped`, and host port 80 mapped to container port 8000.
- **Final public smoke:** `http://121.40.246.197/` returned `200` with the expected WebUI; the exact deterministic demo JSON passed; credential write returned `403` without secret echo; and a local-repository task returned `400` with the expected public-demo denial. Unsolicited Internet probes for `/.env.local` and `/html/ie.html` appeared in the access log and received `404`; no sensitive file was served.
- **Remaining delivery gates:** require the audit-only head's final CI pass, update Pull Request #1 without merging, and leave `REFLECTION.md` student-authored. This audit-only change does not alter `README.md`, `src/`, `demo/`, dependencies, or the Dockerfile inputs, so the running final CI artifact remains the delivered product image.
