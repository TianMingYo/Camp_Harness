from __future__ import annotations

from datetime import UTC, datetime
from typing import Protocol

from feedbackloop.context import ContextBuilder, FileSummary
from feedbackloop.feedback import FeedbackClassifier
from feedbackloop.llm import LLMClient, LLMProviderError
from feedbackloop.models import (
    Action,
    ActionStatus,
    Approval,
    ApprovalDecision,
    CommandResult,
    DomainModel,
    Feedback,
    FeedbackKind,
    IterationRecord,
    Task,
    TaskEvent,
    TaskState,
    ValidationCommand,
)
from feedbackloop.policy import DecisionKind, PolicyEngine
from feedbackloop.state import TaskStateMachine
from feedbackloop.store import Store
from feedbackloop.workspace import Workspace


class LoopExecutor(Protocol):
    workspace: Workspace

    def apply(self, action: Action) -> None: ...
    def validate(self, command: ValidationCommand) -> CommandResult: ...
    def fingerprint(self) -> str: ...
    def file_summaries(self) -> list[FileSummary]: ...


class TaskResult(DomainModel):
    task_id: str
    state: TaskState
    iterations: tuple[IterationRecord, ...]
    pending_approval: Approval | None = None


class FeedbackLoop:
    def __init__(
        self,
        *,
        task_store: Store,
        llm: LLMClient,
        policy: PolicyEngine,
        executor: LoopExecutor,
        classifier: FeedbackClassifier,
        context_builder: ContextBuilder,
        max_iterations: int = 5,
    ) -> None:
        if max_iterations < 1:
            raise ValueError("max_iterations must be positive")
        self.store = task_store
        self.llm = llm
        self.policy = policy
        self.executor = executor
        self.classifier = classifier
        self.context_builder = context_builder
        self.max_iterations = max_iterations

    def approve_plan(self, task_id: str) -> TaskState:
        task = self._task(task_id)
        plan = self.store.get_plan(task_id)
        if plan is None:
            raise ValueError("task has no plan to approve")
        machine = TaskStateMachine(task.state)
        state = machine.transition(TaskEvent.PLAN_APPROVED)
        self.store.save_plan(
            task_id, plan.model_copy(update={"approved_at": datetime.now(UTC)})
        )
        self._update_state(task, state)
        return state

    def run(self, task_id: str) -> TaskResult:
        task = self._task(task_id)
        if task.state is not TaskState.RUNNING:
            raise ValueError("task must have an approved plan before execution")
        plan = self.store.get_plan(task_id)
        if plan is None:
            raise ValueError("task has no approved plan")

        machine = TaskStateMachine(task.state)
        existing_iterations = self.store.list_iterations(task_id)
        previous_feedback = (
            existing_iterations[-1].feedback if existing_iterations else None
        )
        unchanged_rounds = self._trailing_unchanged_rounds(existing_iterations)
        limit = min(task.max_iterations, self.max_iterations)

        for number in range(len(existing_iterations) + 1, limit + 1):
            try:
                before = self.executor.fingerprint()
                context = self.context_builder.build(
                    task,
                    plan,
                    self.store.list_iterations(task_id),
                    self.executor.file_summaries(),
                )
            except Exception as error:
                return self._runtime_failure(
                    task, machine, number, "unavailable", error
                )
            try:
                response = self.llm.complete(context)
            except LLMProviderError as error:
                feedback = Feedback(
                    kind=FeedbackKind.COMMAND_ERROR,
                    summary=str(error),
                )
                record = self._record(task_id, number, before, feedback)
                feedback = feedback.model_copy(update={"iteration_id": record.id})
                record = record.model_copy(update={"feedback": feedback})
                self._persist_iteration(record, feedback, [])
                state = machine.transition(TaskEvent.FAIL)
                self._update_state(task, state)
                return self._result(task_id, state)
            evaluated_actions: list[tuple[Action, DecisionKind, str]] = []

            for proposed in response.actions:
                try:
                    decision = self.policy.check(proposed, self.executor.workspace)
                except Exception as error:
                    return self._runtime_failure(task, machine, number, before, error)
                action = proposed.model_copy(update={"iteration_id": None})
                evaluated_actions.append((action, decision.kind, decision.reason))

            denied = next(
                (
                    (action, reason)
                    for action, kind, reason in evaluated_actions
                    if kind is DecisionKind.DENY
                ),
                None,
            )
            if denied is not None:
                denied_action, reason = denied
                feedback = Feedback(
                    iteration_id=None,
                    kind=FeedbackKind.POLICY_BLOCKED,
                    summary=reason,
                )
                record = self._record(task_id, number, before, feedback)
                feedback = feedback.model_copy(update={"iteration_id": record.id})
                record = record.model_copy(update={"feedback": feedback})
                actions = [
                    action.model_copy(
                        update={
                            "iteration_id": record.id,
                            "status": (
                                ActionStatus.DENIED
                                if action.id == denied_action.id
                                else ActionStatus.PROPOSED
                            ),
                        }
                    )
                    for action, _, _ in evaluated_actions
                ]
                self._persist_iteration(record, feedback, actions)
                state = machine.transition(TaskEvent.FAIL)
                self._update_state(task, state)
                return self._result(task_id, state)

            approval_items = [
                (action, reason)
                for action, kind, reason in evaluated_actions
                if kind is DecisionKind.REQUIRE_APPROVAL
            ]
            if len(approval_items) > 1:
                feedback = Feedback(
                    kind=FeedbackKind.POLICY_BLOCKED,
                    summary="multiple approval-required actions must be split across iterations",
                )
                record = self._record(task_id, number, before, feedback)
                feedback = feedback.model_copy(update={"iteration_id": record.id})
                record = record.model_copy(update={"feedback": feedback})
                actions = [
                    action.model_copy(
                        update={"iteration_id": record.id, "status": ActionStatus.DENIED}
                    )
                    for action, _, _ in evaluated_actions
                ]
                self._persist_iteration(record, feedback, actions)
                state = machine.transition(TaskEvent.FAIL)
                self._update_state(task, state)
                return self._result(task_id, state)

            record = IterationRecord(
                task_id=task_id,
                number=number,
                workspace_fingerprint=before,
            )
            self.store.save_iteration(record)
            prepared_actions = [
                action.model_copy(update={"iteration_id": record.id})
                for action, _, _ in evaluated_actions
            ]
            for action in prepared_actions:
                self.store.save_action(action)

            completed_actions: list[Action] = []
            for action in prepared_actions:
                if approval_items:
                    continue
                try:
                    self.executor.apply(action)
                except Exception as error:
                    failed_action = action.model_copy(
                        update={
                            "status": ActionStatus.FAILED,
                            "result_summary": str(error) or type(error).__name__,
                        }
                    )
                    self.store.update_action(failed_action)
                    self.store.append_audit_event(
                        task.id, "action_failed", {"action_id": action.id}
                    )
                    current_fingerprint = self._current_fingerprint_or(before)
                    return self._runtime_failure(
                        task,
                        machine,
                        number,
                        current_fingerprint,
                        error,
                        record=record,
                        progressed=(
                            bool(completed_actions) or current_fingerprint != before
                        ),
                    )
                completed = action.model_copy(update={"status": ActionStatus.COMPLETED})
                self.store.update_action(completed)
                self.store.append_audit_event(
                    task.id, "action_completed", {"action_id": action.id}
                )
                completed_actions.append(completed)
            prepared_actions = completed_actions

            if approval_items:
                approval_action, reason = approval_items[0]
                attached_action = next(
                    action for action in self.store.list_actions(record.id)
                    if action.id == approval_action.id
                )
                feedback = Feedback(
                    iteration_id=record.id,
                    kind=FeedbackKind.POLICY_BLOCKED,
                    summary=reason,
                )
                current_fingerprint = self._current_fingerprint_or(before)
                record = record.model_copy(
                    update={
                        "workspace_fingerprint": current_fingerprint,
                        "feedback": feedback,
                        "progressed": current_fingerprint != before,
                        "ended_at": datetime.now(UTC),
                    }
                )
                self.store.update_iteration(record)
                self.store.save_feedback(feedback)
                approval = Approval(action_id=attached_action.id, reason=reason)
                self.store.save_approval(approval)
                state = machine.transition(TaskEvent.APPROVAL_REQUIRED)
                self._update_state(task, state)
                return self._result(task_id, state, approval)

            try:
                after = self.executor.fingerprint()
            except Exception as error:
                return self._runtime_failure(
                    task,
                    machine,
                    number,
                    "unavailable",
                    error,
                    record=record,
                    progressed=bool(completed_actions),
                )
            try:
                command_result = self._validate(task.validation_commands)
            except Exception as error:
                return self._runtime_failure(
                    task, machine, number, after, error, record=record
                )
            feedback = self.classifier.classify(command_result, before, after)
            equivalent = (
                before == after
                and previous_feedback is not None
                and previous_feedback.kind is feedback.kind
                and previous_feedback.summary == feedback.summary
            )
            unchanged_rounds = unchanged_rounds + 1 if equivalent else (1 if before == after else 0)
            if unchanged_rounds >= 2:
                feedback = feedback.model_copy(
                    update={
                        "kind": FeedbackKind.NO_PROGRESS,
                        "summary": f"no progress after repeated result\n{feedback.summary}",
                    }
                )

            feedback = feedback.model_copy(update={"iteration_id": record.id})
            record = record.model_copy(
                update={
                    "workspace_fingerprint": after,
                    "result_code": feedback.exit_code,
                    "feedback": feedback,
                    "progressed": before != after,
                    "ended_at": datetime.now(UTC),
                }
            )
            self.store.update_iteration(record)
            self.store.save_feedback(feedback)
            previous_feedback = feedback

            if feedback.kind is FeedbackKind.PASS:
                validation_state = machine.transition(TaskEvent.VALIDATION_PASSED)
                self._update_state(task, validation_state)
                state = machine.transition(TaskEvent.MARK_SUCCEEDED)
                self._update_state(task, state)
                return self._result(task_id, state)
            if unchanged_rounds >= 2:
                state = machine.transition(TaskEvent.NO_PROGRESS)
                self._update_state(task, state)
                self.store.append_audit_event(
                    task.id, "stop_no_progress", {"iterations": number}
                )
                return self._result(task_id, state)
            if feedback.kind in {FeedbackKind.TIMEOUT, FeedbackKind.COMMAND_ERROR}:
                state = machine.transition(TaskEvent.FAIL)
                self._update_state(task, state)
                return self._result(task_id, state)

        state = machine.transition(TaskEvent.MAX_ITERATIONS_REACHED)
        self._update_state(task, state)
        self.store.append_audit_event(
            task.id,
            "stop_max_iterations",
            {"iterations": len(self.store.list_iterations(task.id))},
        )
        return self._result(task_id, state)

    def resolve_approval(
        self, approval_id: str, decision: ApprovalDecision
    ) -> TaskState:
        approval = self.store.get_approval_by_id(approval_id)
        if approval is None:
            raise KeyError(approval_id)
        action = self.store.get_action(approval.action_id)
        if action is None or action.iteration_id is None:
            raise ValueError("approval action is not attached to an iteration")
        iteration = self.store.get_iteration(action.iteration_id)
        if iteration is None:
            raise ValueError("approval iteration is missing")
        task = self._task(iteration.task_id)
        event = (
            TaskEvent.APPROVAL_GRANTED
            if decision is ApprovalDecision.ALLOWED
            else TaskEvent.APPROVAL_DENIED
        )
        state = TaskStateMachine(task.state).transition(event)
        decided = approval.model_copy(
            update={"decision": decision, "decided_at": datetime.now(UTC)}
        )
        self.store.save_approval(decided)
        if decision is ApprovalDecision.ALLOWED:
            self._update_state(task, state)
            failed_action = action
            try:
                for batch_action in self.store.list_actions(iteration.id):
                    if batch_action.status is ActionStatus.COMPLETED:
                        continue
                    failed_action = batch_action
                    self.executor.apply(batch_action)
                    self.store.update_action(
                        batch_action.model_copy(
                            update={"status": ActionStatus.COMPLETED}
                        )
                    )
                    self.store.append_audit_event(
                        task.id,
                        "action_completed",
                        {"action_id": batch_action.id},
                    )
            except Exception as error:
                self.store.update_action(
                    failed_action.model_copy(
                        update={
                            "status": ActionStatus.FAILED,
                            "result_summary": str(error) or type(error).__name__,
                        }
                    )
                )
                failed = TaskStateMachine(state).transition(TaskEvent.FAIL)
                self._update_state(task, failed)
                self.store.append_audit_event(
                    task.id,
                    "approval_execution_failed",
                    {"approval_id": approval.id, "action_id": failed_action.id},
                )
                raise ValueError("approved action execution failed") from error
            self.store.append_audit_event(
                task.id,
                "approval_allowed",
                {"approval_id": approval.id, "action_id": action.id},
            )
        else:
            self.store.update_action(
                action.model_copy(update={"status": ActionStatus.DENIED})
            )
            self._update_state(task, state)
            self.store.append_audit_event(
                task.id,
                "approval_denied",
                {"approval_id": approval.id, "action_id": action.id},
            )
        return state

    def _validate(self, commands: tuple[ValidationCommand, ...]) -> CommandResult:
        if not commands:
            return CommandResult(
                kind="validation",
                error="no validation command is configured",
            )
        result: CommandResult | None = None
        for command in commands:
            result = self.executor.validate(command)
            if result.timed_out or result.error or result.exit_code != 0:
                return result
        assert result is not None
        return result

    def _record(
        self,
        task_id: str,
        number: int,
        fingerprint: str,
        feedback: Feedback,
        *,
        progressed: bool = False,
    ) -> IterationRecord:
        return IterationRecord(
            task_id=task_id,
            number=number,
            workspace_fingerprint=fingerprint,
            result_code=feedback.exit_code,
            feedback=feedback,
            progressed=progressed,
            ended_at=datetime.now(UTC),
        )

    def _persist_iteration(
        self,
        record: IterationRecord,
        feedback: Feedback,
        actions: list[Action],
    ) -> None:
        self.store.save_iteration(record)
        for action in actions:
            self.store.save_action(action)
        self.store.save_feedback(feedback)

    def _runtime_failure(
        self,
        task: Task,
        machine: TaskStateMachine,
        number: int,
        fingerprint: str,
        error: Exception,
        *,
        record: IterationRecord | None = None,
        progressed: bool | None = None,
    ) -> TaskResult:
        feedback = Feedback(
            kind=FeedbackKind.COMMAND_ERROR,
            summary=str(error) or type(error).__name__,
        )
        existing_record = record
        record = record or self._record(task.id, number, fingerprint, feedback)
        feedback = feedback.model_copy(update={"iteration_id": record.id})
        record = record.model_copy(
            update={
                "workspace_fingerprint": fingerprint,
                "feedback": feedback,
                "progressed": (
                    record.workspace_fingerprint != fingerprint
                    if progressed is None
                    else progressed
                ),
                "ended_at": datetime.now(UTC),
            }
        )
        if existing_record is None:
            self._persist_iteration(record, feedback, [])
        else:
            self.store.update_iteration(record)
            self.store.save_feedback(feedback)
        state = machine.transition(TaskEvent.FAIL)
        self._update_state(task, state)
        return self._result(task.id, state)

    def _current_fingerprint_or(self, fallback: str) -> str:
        try:
            return self.executor.fingerprint()
        except Exception:
            return fallback

    @staticmethod
    def _trailing_unchanged_rounds(
        iterations: list[IterationRecord],
    ) -> int:
        count = 0
        signature: tuple[FeedbackKind, str] | None = None
        for record in reversed(iterations):
            if record.progressed or record.feedback is None:
                break
            current = (record.feedback.kind, record.feedback.summary)
            if signature is None:
                signature = current
            elif current != signature:
                break
            count += 1
        return count

    def _task(self, task_id: str) -> Task:
        task = self.store.get_task(task_id)
        if task is None:
            raise KeyError(task_id)
        return task

    def _update_state(self, task: Task, state: TaskState) -> None:
        current = self._task(task.id)
        if current.state is state:
            return
        self.store.update_task(current.model_copy(update={"state": state}))
        self.store.append_audit_event(
            task.id,
            "state_transition",
            {"from": current.state.value, "to": state.value},
        )

    def _result(
        self,
        task_id: str,
        state: TaskState,
        pending: Approval | None = None,
    ) -> TaskResult:
        return TaskResult(
            task_id=task_id,
            state=state,
            iterations=tuple(self.store.list_iterations(task_id)),
            pending_approval=pending,
        )
